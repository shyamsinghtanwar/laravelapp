                ...
                <div class="col-md-12">
                    <div class="table-custom">
                        <table class="table table-striped">
                            ...
                            @forelse($totalamount as $k=>$totalamounts)
                                <tr class="result-table-tr">
                                    <td scope="row">{{++$k}}</td>
                                    <td>
                                    @php
                                    $key = array_search($totalamounts->website_id , array_column($website_data, 'id'));

                                    if($key!== false){
                                     $url   = $website_data[$key]['images'];

                                    echo "<img src='/$url' alt='$url' style='width:250px;height:42px;'>";
                                       // echo $url ;
                                    }
                                    @endphp
                                    </td>
                                    <td><a href="webiste-money-withdraw/{{$totalamounts->website_id}}">Withdraw</a></td>
                                </tr>
