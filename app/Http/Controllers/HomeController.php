...
class HomeController extends Controller
{
    ...
    $data['matches'] =  DB::table('web_user_match')
        ->select(DB::raw('web_user_match.status, web_user_match.id, web_user_match.user_id , web_user_match.website_id ,web_user_match.website_user_name ,web_user_match.website_user_password ,web_user_match.created_at ,web_user_match.amount ,  users.username, events.url,events.images'))
        ->join('users', 'users.id', '=', 'web_user_match.user_id')
        ->join('events','events.id','=','web_user_match.website_id')
        ->where('web_user_match.user_id',$user)
        ->get();

        $data['totalamount'] =  Web_user_match::groupBy('website_id')
        ->selectRaw('sum(amount) as sum, website_id')
        ->where('web_user_match.status', 1)
        ->where('web_user_match.website_user_name','!=', 'Withdraw_Request')
        ->where('web_user_match.user_id', $user)
        ->get();

        $data['website_data']=Event::get()->toArray();
    ...
    $data['matches'] =  DB::table('web_user_match')
        ...
        return view('user.my-prediction-withdrawtrans', $data);
    }
    ...
    {
        $data['page_title'] = "Get ID of Website";
        $user = Auth::user()->id;
        $data['website'] =  Event::get();
        $data['site_user_disable'] = Web_user_match::where('user_id',$user)->groupBy('website_id')->get()->toArray();
        return view('user.websiteid', $data);
    }

    public function processid(Request $request)
    {
        $data['page_title'] = "Process ID";
        $user = Auth::user()->id;
        $website_id = $request->uid;

        $web_user_match = new Web_user_match;

        $web_user_match->user_id = $user;
        $web_user_match->website_id = $website_id;
        $web_user_match->website_user_name = 'Null';
        $web_user_match->website_user_password= 'Null';
        $web_user_match->amount =0;
        $web_user_match->status =0;
        $web_user_match->save();

            $phone = User::where('id', $user)->value('phone');
            ...
            [
                ...
                'body' => "New ID Request raised"]);

        event(new MyEvent('New ID Request recieved'));

        session()->flash('success', 'Request Rasied Successfully');
        return back();
    ...
    public function websitemoneyadd()
    {
        $data['page_title'] = "Add Amount to a Website";
        $user = Auth::user()->id;
        $data['user_id'] = $user;
        $data['bal'] = User::where('id', $user)->value('balance');
        $data['site_user_disable'] = Web_user_match::where('user_id',$user)->where('status',1)->groupBy('website_id')->get()->toArray();
        $data['events'] = Event::get();

        return view('user.website-money-transfer', $data);
    }




    public function websitemoneywithdraw(request $request)
    {
        $data['page_title'] = "Withdraw Money from a Website";
        $user = Auth::user()->id;
        $data['user_id'] = $user;

        $data['bal'] = User::where('id', $user)->value('balance');
       $data['validation_check'] = Web_user_match::join('events','events.id','=', 'web_user_match.website_id' )
       ->where('web_user_match.user_id',$user)
       ->where('web_user_match.website_user_name','!=', 'Null')
       ->where('web_user_match.website_user_password','!=', 'Null')
       ->where('web_user_match.amount','>', 0)
       ->where('web_user_match.status', 1)
       ->where('web_user_match.website_id', $request->id)
       ->first();



        return view('user.website-money-withdraw', $data);
    }


    public function websitemoneyaddconfirm(Request $request)
    {
        ...
        $user = Auth::user()->id;
        $bal = User::where('id', $user)->value('balance');
        $website_details= Event::where('id',$request->website)->first();
        $user_password = Web_user_match::where('user_id',$user)->where('website_id',$request->website)->where('status',1)->first();
        ...
        if($bal>$investment){
            ...
            $web_user_match->user_id = $user;
            $web_user_match->website_id = $request->website;
            $web_user_match->website_user_name = $user_password->website_user_name;
            $web_user_match->website_user_password= $user_password->website_user_password;
            $web_user_match->amount =$investment;
            $web_user_match->status = 0;

            $web_user_match->save();

            $new_balance = $bal -  $investment;
            ...
            [
                'from' => env('TWILIO_SMS_NUMBER'),
                'body' => "Request Accepted to Add Money"]);


            event(new MyEvent('Add New Money to Website'));

            session()->flash('success', 'Money Added Successfully and Waiting for Approval');
    ...
    public function websitemoneywithdrawconfirm(Request $request)
    {
        ...
        $validation_check = Web_user_match::join('events','events.id','=', 'web_user_match.website_id' )
        ->where('web_user_match.user_id',$user)
        ->where('web_user_match.website_user_name','!=', 'Null')
        ->where('web_user_match.website_user_password','!=', 'Null')
        ->where('web_user_match.amount','>', 0)
        ->where('web_user_match.status', 1)
        ->where('web_user_match.website_id', $request->website_id)
        ->count();
        ...
        if($validation_check>0){
            ...
            $web_user_match->user_id = $user;
            $web_user_match->website_id = $request->website_id;
            $web_user_match->website_user_name = 'Withdraw_Request';
            $web_user_match->website_user_password= 'Withdraw_Request';
            $web_user_match->amount =$withdrawal_amount;
            $web_user_match->status = 0;
            ...
            [
                'from' => env('TWILIO_SMS_NUMBER'),
                'body' => "New Withdrawal Request raised"]);

            event(new MyEvent('New Withdrawal Request'));

            session()->flash('success', 'Withdraw Request raised and Waiting for Approval');
            return back()->withInput();
        }else{
            session()->flash('Failed', 'Invalid Operation!!');
            return back()->withInput();
        }
    ...
    public function withdrawReqSubmit(Request $request)
    {
        ...
        if (formatter_money($withdraw->net_amount) > $user->balance) {
            ...
            [
                'from' => env('TWILIO_SMS_NUMBER'),
                'body' => "Money Withdrawal request Recieved"]);
